...
public class ClientListenerHandler extends SimpleChannelInboundHandler<MessageBean> {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        log.info("客户端监听：{} 连上了服务器", ctx.channel().remoteAddress());
    }
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        log.info("客户端监听：{} 断开了服务器", ctx.channel().remoteAddress());
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, MessageBean messageBean) throws Exception {
        log.info("客户端监听：来自服务端的消息:{}", new String(messageBean.getContent(), CharsetUtil.UTF_8));
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        log.error("客户端监听：{} 连接出异常了", ctx.channel().remoteAddress());
